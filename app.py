...
def _by_filter(path, вид):
    """Расширение — по выбранному в окне виду файла.

    macOS не переписывает расширение, которое уже стоит в имени: имя мы
    предлагаем с «.md», и при выборе «Текст» файл всё равно сохранялся
    разметкой.
    """
    виды = re.findall(r"\*(\.\w+)", вид or "")
    return (str(Path(path).with_suffix(виды[0]))
            if виды and Path(path).suffix.lower() not in виды else path)
...
class Window(QMainWindow):
    ...
    def save_file(self):
        ...
        if self.path is None or self.path.suffix.lower() in papers.READ_ONLY:
            ...
            path, вид = QFileDialog.getSaveFileName(
                self, t("Сохранить текст"), str(start), t(FILTER_SAVE))
            if not path:
                return
            self.path = Path(_by_filter(path, вид))
        try:
